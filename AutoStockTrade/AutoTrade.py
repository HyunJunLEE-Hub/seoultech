...
slack = Slacker('your slack oauth')# 슬랙 oauth
def dbgout(message):
    """인자로 받은 문자열을 파이썬 셸과 슬랙으로 동시에 출력한다."""
    print(datetime.now().strftime('[%m/%d %H:%M:%S]'), message)
    strbuf = datetime.now().strftime('[%m/%d %H:%M:%S] ') + message
    slack.chat.post_message('#stock', strbuf) # 슬렉 채널명
...
def get_target_price(code):
    ...
    try:
        ...
        target_price = today_open + (lastday_high - lastday_low) * 0.5 # 변동성 돌파 전략 / 지난날의 고가와 지난날의 저가의 차이를 계산
        return target_price
...
if __name__ == '__main__': 
    try:
        symbol_list = ['A122630', 'A252670', 'A233740', 'A250780', 'A225130',
             'A280940', 'A261220', 'A217770', 'A295000', 'A176950'] # 매수하고자 하는 종목코드 리스트
        bought_list = []     # 매수 완료된 종목 리스트
        target_buy_count = 5 # 리스트 중 매수할 종목 수
        buy_percent = 0.19 # 보유한 금액에서 몇 퍼센트를 살건지 ex) 0.19 * 5
        ...
        while True:
            t_now = datetime.now() # 시간계산
            t_9 = t_now.replace(hour=9, minute=0, second=0, microsecond=0) 
            t_start = t_now.replace(hour=9, minute=5, second=0, microsecond=0) # 09:05 stats
            t_sell = t_now.replace(hour=15, minute=15, second=0, microsecond=0) # 15:15 sell
            t_exit = t_now.replace(hour=15, minute=20, second=0,microsecond=0) # 프로그램 종료
            ...
            if t_start < t_now < t_sell :  # AM 09:05 ~ PM 03:15 : 매수
                for sym in symbol_list: # 매수할 종목들 하나씩
